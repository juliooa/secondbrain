<script lang="ts">
	...
	async function downloadModel() {
		console.log('Selected model id:' + selectedModelToDownloadId);
	...
	<div class="flex flex-col justify-between grow mt-4">
		<div>
			<label class="label">
				...
				<input class="input" disabled type="text" placeholder="Input" value="Vicunaaa" />
			</label>
			<label class="label mt-4">
				...
				<select class="select" size="6" bind:value={selectedModelId}>
					{#each languageModels.filter((model) => model.downloaded) as model}
						<option value={model.id} disabled={model.default}>{model.name}</option>
					{/each}
				</select>
				<button type="button" class="btn variant-filled-secondary">Select model</button>
			</label>
			<label class="label mt-4">
